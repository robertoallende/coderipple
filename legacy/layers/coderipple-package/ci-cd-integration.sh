#!/bin/bash
# layers/coderipple-package/ci-cd-integration.sh
# CI/CD integration script for CodeRipple Package Layer
...
log_section "CodeRipple Package Layer - CI/CD Integration"
...
pre_build_validation() {
    ...
    # Check CodeRipple source package
    CODERIPPLE_SOURCE="../../coderipple"
    if [ ! -f "$CODERIPPLE_SOURCE/setup.py" ]; then
        log_error "CodeRipple source package not found: $CODERIPPLE_SOURCE/setup.py"
        exit 1
    fi
    log_success "CodeRipple source package found"
    
    # Check required build scripts
    required_scripts=("1-install.sh" "2-package.sh" "3-validate.sh")
    for script in "${required_scripts[@]}"; do
        if [ ! -f "$script" ]; then
            log_error "Required build script missing: $script"
            exit 1
        fi
    done
    log_success "Required build scripts validated"
    
    # Check disk space (need at least 500MB for package build)
...
ci_optimized_build() {
    ...
    if [ ! -f "coderipple-package-layer.zip" ]; then
        log_error "Build failed: package layer ZIP not created"
        exit 1
    fi
    
    layer_size=$(du -sh "coderipple-package-layer.zip" | cut -f1)
    log_success "Build completed: package layer size $layer_size"
}
...
manage_artifacts() {
    ...
    # Copy key artifacts
    cp "coderipple-package-layer.zip" "artifacts/"
    ...
    cat > "artifacts/manifest.json" << EOF
{
  ...
  "artifacts": [
    {
      "name": "coderipple-package-layer.zip",
      "type": "lambda_layer",
      "size": "$(du -b coderipple-package-layer.zip | cut -f1)",
      "checksum": "$(shasum -a 256 coderipple-package-layer.zip | cut -d' ' -f1)"
    }
  ]
}
EOF
...
upload_artifacts() {
    ...
    case "$CI_ENVIRONMENT" in
        "github-actions")
            ...
            if [ -n "$GITHUB_OUTPUT" ]; then
                echo "package_layer_zip=artifacts/coderipple-package-layer.zip" >> "$GITHUB_OUTPUT"
                echo "package_layer_size=$(du -sh artifacts/coderipple-package-layer.zip | cut -f1)" >> "$GITHUB_OUTPUT"
            fi
            ;;
        "gitlab-ci")
...
# CodeRipple Package Layer Build Summary
...
## Build Results
- **Package Layer Size**: $layer_size ($layer_size_bytes bytes)
- **CodeRipple Version**: $coderipple_version
- **Build Mode**: $BUILD_MODE
- **Validation**: $([ -f "validation-report.json" ] && echo "✅ Passed" || echo "⚠️ Skipped")

## Package Contents
- **Custom Agents**: Tourist Guide, Building Inspector, Historian, Orchestrator
- **Supporting Tools**: Configuration, Webhook Parser, Git Analysis, Content Generation
- **Module Count**: $(find build/python -name "*.py" -type f 2>/dev/null | wc -l) Python modules

## Artifacts Generated
- \`coderipple-package-layer.zip\` - Lambda layer package
- \`build-report.json\` - Detailed build metrics
- \`validation-report.json\` - Validation results
- \`ci-summary.md\` - This summary

## Next Steps
1. Deploy package layer to AWS Lambda
2. Update Lambda functions to use both dependency and package layers
3. Test complete layer-based architecture
...
EOF
    ...
    echo ""
    echo "📋 Build Summary:"
    echo "   Package Layer: $layer_size"
    echo "   CodeRipple Version: $coderipple_version"
...
main() {
    ...
    echo ""
    echo "🎉 CodeRipple Package Layer CI/CD Build Complete!"
    echo ""
    echo "📊 Summary:"
    echo "   Environment: $CI_ENVIRONMENT"
    echo "   Package Layer Size: $(du -sh coderipple-package-layer.zip | cut -f1)"
