"""
Import Diagnostics Test - Comprehensive validation of CodeRipple package imports

This test provides detailed diagnostics when imports fail, including:
- Environment analysis (Python path, working directory, package locations)
- Available modules discovery and listing
- Specific import failure reporting with suggested fixes
...
def test_import_diagnostics():
    """Main diagnostic test that provides comprehensive import analysis"""
    ...
    if diagnostics.failed_imports:
        failure_details = []
        for failure in diagnostics.failed_imports:
            failure_details.append(f"{failure['module']}: {failure['error']}")
            
        pytest.fail(
            f"Import diagnostics found {len(diagnostics.failed_imports)} failed imports:\n" +
            "\n".join(failure_details) + 
            f"\n\nSee diagnostic report above for detailed analysis and recommendations."
        )
    
    # Verify we have the core modules
    core_modules = [
        'coderipple.orchestrator_agent',
        'coderipple.tourist_guide_agent', 
        'coderipple.building_inspector_agent',
    ...
    missing_core = [mod for mod in core_modules if mod not in diagnostics.successful_imports]
    if missing_core:
        pytest.fail(f"Missing core CodeRipple modules: {missing_core}")
        
    print(f"\n✅ All import diagnostics passed! Successfully imported {len(diagnostics.successful_imports)} modules.")

def test_specific_agent_imports():
    """Test specific agent imports with detailed error reporting"""
    agents = {
        'orchestrator_agent': 'coderipple.orchestrator_agent',
        'tourist_guide_agent': 'coderipple.tourist_guide_agent',
        'building_inspector_agent': 'coderipple.building_inspector_agent', 
        'historian_agent': 'coderipple.historian_agent'
    }
    
    for agent_name, module_path in agents.items():
        try:
            module = importlib.import_module(module_path)
            print(f"✅ {agent_name}: {module_path}")
            
            # Try to access main function if it exists
            if hasattr(module, agent_name):
                func = getattr(module, agent_name)
                print(f"   📋 Main function '{agent_name}' available: {callable(func)}")
                
        except ImportError as e:
            pytest.fail(f"Failed to import {agent_name} from {module_path}: {e}")

def test_tool_imports():
    """Test tool imports with detailed error reporting"""
    tools = {
        'webhook_parser': 'coderipple.webhook_parser',
        'git_analysis_tool': 'coderipple.git_analysis_tool',
        'config': 'coderipple.config',
        'content_generation_tools': 'coderipple.content_generation_tools',
        'bedrock_integration_tools': 'coderipple.bedrock_integration_tools'
    }
    
    for tool_name, module_path in tools.items():
        try:
            module = importlib.import_module(module_path)
            print(f"✅ {tool_name}: {module_path}")
        except ImportError as e:
            pytest.fail(f"Failed to import {tool_name} from {module_path}: {e}")
