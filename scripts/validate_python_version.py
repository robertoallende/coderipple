...
Ensures Python 3.13.x is being used for consistency with AWS Lambda runtime.
"""
...
def check_python_version():
    ...
    if current_version < REQUIRED_PYTHON:
        ...
        print(f"   CodeRipple requires Python 3.13.x for:")
        print(f"   • AWS Lambda runtime compatibility (python3.13)")
        print(f"   • Strands Agents SDK compatibility (requires 3.10+)")
        print(f"   • Latest performance and security improvements")
        print(f"")
        print(f"   Please install Python 3.13.x from https://python.org/downloads/")
        sys.exit(1)
